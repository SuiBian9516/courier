use syn::{Attribute, Expr, GenericParam, Generics, Lit, Meta, Token, Type, TypeParamBound, punctuated::Punctuated};
...
pub fn get_rename(attrs: &[Attribute], ident_name: String) -> String {
  for attr in attrs {
    ...
    match &attr.meta {
      Meta::List(list) => {
        for meta in list.parse_args_with(Punctuated::<Meta, Token![,]>::parse_terminated).unwrap_or_default() {
